...
const MedicalRecordsPage = () => {
  ...
  // Effect 1: Lấy danh sách bệnh nhân (CHO BÁC SĨ)
  useEffect(() => {
  ...
  // Effect 2: Lấy chi tiết khi bác sĩ chọn bệnh nhân
  useEffect(() => {
    if (userRole?.toLowerCase() === "doctor" && selectedPatient?.id) {
      scrollToTop();
  ...
  // THÊM MỚI: Effect 3: Lấy lịch sử (CHO BỆNH NHÂN)
  useEffect(() => {
    if (userRole?.toLowerCase() === "patient" && patientProfile?.profileId) {
      const fetchPatientOwnHistory = async () => {
        ...
        try {
          const [historyData, patientProfileData] = await Promise.all([
            getPatientHistory(patientProfile.profileId),
            getPatientById(patientProfile.profileId),
          ]);
          setHistory(historyData);
          setDetailedPatient(patientProfileData);
        } catch (error) {
          console.error("Failed to fetch patient's own history:", error);
        } finally {
          setLoadingHistory(false);
        }
      };
  ...
  if (userRole?.toLowerCase() === "patient") {
    return (
      <div className="min-h-screen w-full bg-gray-100 p-4 md:p-8 font-sans text-gray-800">
        <main className="max-w-4xl mx-auto">
          {/* Thông tin chi tiết bệnh nhân */}
          {detailedPatient ? (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
              <div className="flex items-start justify-between mb-6 border-b border-gray-300 pb-4">
                <div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 animate-pulse mb-8">
          ...
          <div>
            ...
            {loadingHistory ? (
              ...
              <div className="space-y-4">
                {/* Copy-paste phần map accordion từ giao diện bác sĩ */}
                {history.map((record) => {
                        ...
                        <div className="border-t border-gray-100 p-5 bg-gray-50/30 rounded-b-xl space-y-6">
                            ...
                            record.testResults.length > 0 && (
                              <div className="space-y-3">
                                <h4 className="flex items-center font-bold text-gray-800 text-sm uppercase tracking-wide">
                                  <FaFlask className="mr-2 text-cyan-600" /> Kết
                                  quả xét nghiệm
                                </h4>
                                {record.testResults.map((test) => (
                                  ...
                                  >
                                    <div className="bg-cyan-50 px-4 py-2 border-b border-cyan-100 flex justify-between items-center">
                                      ...
                                      <span className="text-xs text-cyan-600 bg-white px-2 py-1 rounded border border-cyan-200 font-medium">
                                        Kết luận: {test.generalConclusion}
                                      </span>
                                    </div>
                          ...
                          {record.prescriptions &&
                            record.prescriptions.length > 0 && (
                              <div className="space-y-3">
                                <h4 className="flex items-center font-bold text-gray-800 text-sm uppercase tracking-wide">
                                  <FaPills className="mr-2 text-rose-600" /> Đơn
                                  thuốc
                                </h4>
                                {record.prescriptions.map((prescription) => (
                                  ...
                                  >
                                    <div className="mb-3 pb-2 border-b border-gray-100">
                                      ...
                                      {prescription.advice && (
                                        <p className="text-sm mt-1 text-rose-700 italic">
          ...
          */}
          ...
          <main className="col-span-1 md:col-span-3">
            {selectedPatient ? (
              <div className="space-y-6 animate-in fade-in duration-500 outline-none border-none">
                ...
                {detailedPatient ? (
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    <div className="flex items-start justify-between mb-6 border-b pb-4 border-gray-300">
                      <div>
                        <h2 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                          {detailedPatient.fullName}
                        </h2>
                              ...
                              <div className="border-t border-gray-100 p-5 bg-gray-50/30 rounded-b-xl space-y-6">
                                ...
                                {record.testResults &&
                                  record.testResults.length > 0 && (
                                    <div className="space-y-3">
                                      <h4 className="flex items-center font-bold text-gray-800 text-sm uppercase tracking-wide">
                                        <FaFlask className="mr-2 text-cyan-600" />{" "}
                                        Kết quả xét nghiệm
                                      </h4>
                                      {record.testResults.map((test) => (
                                        <div
                                ...
                                {record.prescriptions &&
                                  record.prescriptions.length > 0 && (
                                    <div className="space-y-3">
                                      <h4 className="flex items-center font-bold text-gray-800 text-sm uppercase tracking-wide">
                                        <FaPills className="mr-2 text-rose-600" />{" "}
                                        Đơn thuốc
                                      </h4>
                                ...
                                <div className="flex justify-center pt-2">
                                  <Button
                                    ...
                                    translate={false}
                                  >
                                    <FaChevronUp className="mr-2 h-3 w-3" /> Thu
                                    gọn
                                  </Button>
                                </div>
                              </div>
              ...
              <div
                className="h-[calc(100vh-32px)] flex flex-col items-center justify-center text-gray-400 outline-none
            bg-white rounded-xl border-2 border-dashed border-gray-200"
              >
                ...
                <p className="text-sm mt-2">
                  Vui lòng chọn một bệnh nhân từ danh sách bên trái để xem chi
                  tiết.
                </p>
              </div>
            )}
